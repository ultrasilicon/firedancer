...
test_s0_handshake( void ) {
...
#endif
#if 0
  uchar payload[SNP_BASIC_PAYLOAD_MTU]; /* FIXME: use the correct MTU here */
  uchar rcv_payload[SNP_BASIC_PAYLOAD_MTU];
  ushort payload_sz = SNP_BASIC_PAYLOAD_MTU;
  long rcv_payload_sz;
  ...
  /*
  snp_endpoint_send_all( payload, ..list_of_dst.. ) {
    if (multicast_enabled) {
      snp_s0_endpoint_send(..., config={ multicast })
    }
    for dst in list_of_dst {
      if dst.is_multicast {
        continue
      }
      snp_s0_endpoint_send(..., config={ })
    }
  }
  */

  long encoded_sz = fd_snp_s0_encode_appdata(client_hs, payload, payload_sz, client_pkt /*, config */);
  assert(encoded_sz > 0L);

  /* client_pkt to net tile -> client_pkt from net tile */

  rcv_payload_sz = fd_snp_s0_decode_appdata(&server_hs, client_pkt, (ushort)encoded_sz, rcv_payload);
...
bench_cookie( void ) {
  ...
  snp_cookie_claims_t const claims = {0};
  uchar const cookie_secret[ SNP_COOKIE_KEY_SZ ] = {0};
  uchar cookie[32];

  /* warmup */
  for( unsigned long rem=1000000UL; rem; rem-- ) {
    snp_cookie_create( cookie, &claims, cookie_secret );
...
bench_cookie_verify( void ) {
  ...
  snp_cookie_claims_t claims = {0};
  uchar const cookie_secret[ SNP_COOKIE_KEY_SZ ] = {0};
  uchar cookie[32] = {0};

  /* warmup */
  for( unsigned long rem=1000000UL; rem; rem-- ) {
    int res = snp_cookie_verify( cookie, &claims, cookie_secret );
    __asm__ __volatile__( "# Compiler Barrier" : "+r" (cookie[0]) );
...
bench_ephemeral_generate( void ) {
  ...
  for( unsigned long rem=1000UL; rem; rem-- ) {
    fd_snp_s0_crypto_key_share_generate( private_key, public_key );
...
bench_enc_state_generate( void ) {
  ...
  /* warmup */
  for( unsigned long rem=1000UL; rem; rem-- ) {
    fd_snp_s0_crypto_enc_state_generate( private_key_enc, public_key, key );
...
bench_enc_state_verify( void ) {
  ...
  /* warmup */
  for( unsigned long rem=1000UL; rem; rem-- ) {
    fd_snp_s0_crypto_enc_state_verify( private_key, private_key_enc, public_key, key );
    __asm__ __volatile__( "# Compiler Barrier" : "+r" (private_key[0]) );
  }
