...
int main( int     argc,
  ...
  fd_funk_t * funk;
  fd_funk_join( &funk, fd_funk_new( fd_wksp_alloc_laddr( wksp, fd_funk_align(), fd_funk_footprint( txn_max, rec_max ), funk_tag ), funk_tag, funk_seed, txn_max, rec_max ) );
  FD_TEST( funk );
  ...
  void * runtime_public_mem = fd_wksp_alloc_laddr( wksp,
    ...
    fd_runtime_public_footprint(4*1024*1024 * 1024UL), FD_EXEC_EPOCH_CTX_MAGIC );
  if( FD_UNLIKELY( !runtime_public_mem ) ) {
    FD_LOG_ERR(( "Unable to allocate runtime_public mem" ));
  }
  fd_memset( runtime_public_mem, 0, fd_runtime_public_footprint(4*1024*1024 * 1024UL) );
  fd_runtime_public_t * runtime_public = fd_runtime_public_join( fd_runtime_public_new( runtime_public_mem, 4*1024*1024 * 1024UL ) );
