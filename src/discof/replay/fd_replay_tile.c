...
//   fd_memcpy( vote_bank_hash, &fork->slot_ctx->slot_bank.banks_hash, sizeof(fd_hash_t) );
...
prepare_new_block_execution( fd_replay_tile_ctx_t * ctx,
  ...
  *prev_slot = fork->slot_ctx->slot;
  ...
  ctx->slot_ctx->bank->tick_height = ctx->slot_ctx->bank->max_tick_height;

  if( FD_UNLIKELY( FD_RUNTIME_EXECUTE_SUCCESS != fd_runtime_compute_max_tick_height( *ticks_per_slot,
                                                                                     curr_slot,
                                                                                     &ctx->slot_ctx->bank->max_tick_height ) ) ) {
  ...
  if( flags & REPLAY_FLAG_PACKED_MICROBLOCK ) {
    ...
    xid.ul[1] = fork->slot_ctx->slot;
  }
  xid.ul[0] = fork->slot_ctx->slot;
...
init_poh( fd_replay_tile_ctx_t * ctx ) {
  ...
  msg->hashcnt_per_tick = ctx->slot_ctx->bank->hashes_per_tick;
  msg->ticks_per_slot   = *(fd_bank_mgr_ticks_per_slot_query( ctx->slot_ctx->bank_mgr ));
  msg->tick_duration_ns = (ulong)(ctx->slot_ctx->bank->ns_per_slot) / *(fd_bank_mgr_ticks_per_slot_query( ctx->slot_ctx->bank_mgr ));
...
prepare_first_batch_execution( fd_replay_tile_ctx_t * ctx, fd_stem_context_t * stem ) {
  ...
  if( ctx->capture_ctx ) {
    fd_solcap_writer_set_slot( ctx->capture_ctx->capture, fork->slot_ctx->slot );
  }
...
read_snapshot( void *              _ctx,
  ...
  if( strcmp( snapshot, "funk" )==0 || strncmp( snapshot, "wksp:", 5 )==0 ) {
    ...
    FD_LOG_WARNING(("BASE SLOT: %lu", base_slot));
...
init_after_snapshot( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !snapshot_slot ) ) {
    ...
    *prev_slot = 0UL;
    ...
    ulong hashcnt_per_slot = ctx->slot_ctx->bank->hashes_per_tick * *(fd_bank_mgr_ticks_per_slot_query( ctx->slot_ctx->bank_mgr ));
...
init_snapshot( fd_replay_tile_ctx_t * ctx,
  ...
  FD_LOG_WARNING(("CURR SLOT %lu", ctx->curr_slot));
...
after_credit( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( flags & EXEC_FLAG_FINISHED_SLOT ) ){
    ...
    if( FD_LIKELY( ctx->tower_out_idx!=ULONG_MAX && !ctx->read_only ) ) {
      uchar * chunk_laddr = fd_chunk_to_laddr( ctx->tower_out_mem, ctx->tower_out_chunk );
      fd_hash_t * bank_hash = fd_bank_mgr_bank_hash_query( ctx->slot_ctx->bank_mgr );
    ...
    *priority_fees = 0UL;
    ...
    fd_hash_t * bank_hash = fd_bank_mgr_bank_hash_query( fork->slot_ctx->bank_mgr );
    fd_bank_hash_cmp_t * bank_hash_cmp = child->slot_ctx->bank_hash_cmp;
