...
#ifndef HEADER_fd_src_discof_kappa_generated_fd_kappa_tile_seccomp_h
#define HEADER_fd_src_discof_kappa_generated_fd_kappa_tile_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_fd_kappa_tile_instr_cnt = 18;

static void populate_sock_filter_policy_fd_kappa_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, uint shreds_fd, uint requests_fd) {
  FD_TEST( out_cnt >= 18 );
...
//  lbl_1:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, shreds_fd, /* RET_ALLOW */ 7, /* lbl_2 */ 0 ),
//  lbl_2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, requests_fd, /* RET_ALLOW */ 5, /* lbl_3 */ 0 ),
//  lbl_3:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
