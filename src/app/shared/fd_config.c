  ...
  if( FD_UNLIKELY( config->development.netns.enabled ) ) {
    ...
    if( FD_UNLIKELY( !config->is_firedancer ) ) {
      if( FD_UNLIKELY( strcmp( config->frankendancer.gossip.host, "" ) ) ) {
        ...
        if( FD_UNLIKELY( strlen( config->frankendancer.gossip.host )<=15UL ) ) {
          /* Only sets gossip_ip_addr if it's a valid IPv4 address, otherwise assume it's a DNS name */
          has_gossip_ip4 = fd_cstr_to_ip4_addr( config->frankendancer.gossip.host, &gossip_ip_addr );
        }
...
fd_config_validate( fd_config_t const * config ) {
  ...
  CFG_HAS_NON_ZERO ( layout.snp_tile_count );
