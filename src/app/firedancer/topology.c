...
//  FOR(resolv_tile_cnt) fd_topob_tile_in(  topo, "resolv",  i,            "metric_in", "replay_resol", 0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  /**/             fd_topos_tile_in_net(  topo,                          "metric_in", "send_net",     0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  /**/                 fd_topob_tile_in ( topo, "eqvoc",  0UL,         "metric_in", "gossip_send",   0UL,    FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED   );
  /**/                 fd_topob_tile_in ( topo, "send",   0UL,         "metric_in", "replay_send",   0UL,    FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED   );
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( !strcmp( tile->name, "net" ) || !strcmp( tile->name, "sock" ) ) ) {
      ...
      tile->send.tpu_listen_port = config->tiles.quic.regular_transaction_listen_port;
